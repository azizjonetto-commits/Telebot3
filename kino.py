...
bot = TeleBot("7733509918:AAHwVCXFEN4s2OqBbagtO9JyBw7b5QvYxjA")
...
def IsSubscribe(chat_id):
    ...
    bot.send_message(chat_id, "Botdan foydalanish uchun kanallarga obuna bo'lishiniz kerak!", reply_markup=markup)

def kinolar(message):
    bot.reply_to(message,f"""Salom!Men Animelarni qisqa treyleylarini topib beruvchi botman.""")
...
def start(message):
    ...
    if check_user(user_id):
        bot.send_message(message.chat.id, "Botdan foydalanishingiz mumikin!")
        kinolar(message)
    else:
...
def send(message):
    ...
    if message.text == "/1":
        ...
        bot.send_video(message.chat.id, b)
        bot.answer_callback_query(call.id)
    elif message.text == "/3":
        c = open("Shadow.mp4","rb")
        bot.send_video(message.chat.id, c)
        bot.answer_callback_query(call.id)
    elif message.text == "/4":
        ...
        bot.send_video(message.chat.id, e)
        bot.answer_callback_query(call.id)
    elif message.text == "/6":
        ...
        bot.send_video(message.chat.id, g)
        bot.answer_callback_query(call.id)
    elif message.text == "/8":
        h = open("Shamolni o'rgatish.mp4","rb")
        bot.send_video(message.chat.id, h)
        bot.answer_callback_query(call.id)
    elif message.text == "/9":
        i = open("Xarobalar qirolligi.mp4","rb")
        bot.send_video(message.chat.id, i)
        bot.answer_callback_query(call.id)
    elif message.text == "/10":
        j = open("Titanlar hujumi.mp4","rb")
        bot.send_video(message.chat.id, j)
        bot.answer_callback_query(call.id)
...
def nimadir(call):
    ...
    if check_user(user_id):
        if call.data == "a":
            ...
            b = open("Blue Lock.mp4","rb")
            bot.send_video(call.message.chat.id, b)
            bot.answer_callback_query(call.id)
        elif call.data == "c":
            c = open("Shadow.mp4","rb")
            bot.send_video(call.message.chat.id, c)
            bot.answer_callback_query(call.id)
        elif call.data == "d":
            d = open("Jodugarlar Jangi.mp4","rb")
            bot.send_video(call.message.chat.id, d)
            bot.answer_callback_query(call.id)   
        elif call.data == "e":
            e = open("Iblis qotili.mp4","rb")
            bot.send_video(call.message.chat.id, e)
            bot.answer_callback_query(call.id)      
        elif call.data == "f":
            f = open("Naruto.mp4","rb")
            ...
            g = open("Quvib o'tish.mp4","rb")
            bot.send_video(call.message.chat.id, g)
            bot.answer_callback_query(call.id) 
        elif call.data == "h":
            h = open("Shamolni o'rgatish.mp4","rb")
            bot.send_video(call.message.chat.id, h)
            bot.answer_callback_query(call.id)
        elif call.data == "i":
            i = open("Xarobalar qirolligi.mp4","rb")
            bot.send_video(call.message.chat.id, i)
            bot.answer_callback_query(call.id)   
        elif call.data == "j":
            j = open("Titanlar hujumi.mp4","rb")
            bot.send_video(call.message.chat.id, j)
            bot.answer_callback_query(call.id)
    else:
